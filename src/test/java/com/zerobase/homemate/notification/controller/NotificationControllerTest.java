...
class NotificationControllerTest {
    ...
    @Nested
    @DisplayName("GET /notifications/notices")
    class GetNotices {

        private static final String PATH = "/notifications/notices";
        ...
        void success() throws Exception {
            // given
            Long userId = 1L;
            List<NoticeDto> result = noticeDtoList.stream()
                    .sorted(Comparator.comparing(NoticeDto::getScheduledAt).reversed())
                    .toList();
            when(notificationService.getNotices(userId)).thenReturn(result);
    ...
    @Nested
    @DisplayName("PATCH /notifications/notices/{notificationId}")
    class ReadNotice {

        private static final String PATH = "/notifications/notices/{notificationId}";
